...
VideoOutput* video_output_new(FlTextureRegistrar* texture_registrar,
  ...
  if (self->configuration.enable_hardware_acceleration) {
    // Flutter does not guarantee that its renderer context is current while a
    // platform-channel method is handled.  Build a temporary GLES context from
    // the actual GTK window when necessary; GDK then selects the same native
    // display and driver as the application without relying on vendor flags.
    GdkGLContext* bootstrap_context = NULL;
    ...
    if (flutter_display == EGL_NO_DISPLAY ||
      ...
      if (window != NULL) {
        bootstrap_context = gdk_window_create_gl_context(window, &error);
      }
      if (bootstrap_context != NULL) {
        gdk_gl_context_set_required_version(bootstrap_context, 3, 2);
        gdk_gl_context_set_use_es(bootstrap_context, TRUE);
        if (gdk_gl_context_realize(bootstrap_context, &error)) {
          gdk_gl_context_make_current(bootstrap_context);
      ...
      if (flutter_display == EGL_NO_DISPLAY ||
          flutter_context == EGL_NO_CONTEXT) {
        g_printerr(
            "media_kit: VideoOutput: GTK could not provide an EGL context%s%s.\n",
            error != NULL ? ": " : "",
    ...
    if (bootstrap_context != NULL) {
      ...
      g_object_unref(bootstrap_context);
    }
  }
