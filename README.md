...
Android release signing is deliberately not allowed to fall back to the debug
key. Copy `android/key.properties.example` to `android/key.properties` and
point it at a private upload keystore for distributable APKs; without that
untracked file, the command is an unsigned release-build verification only.
...
`pubspec.yaml` contains the dependencies the app intentionally requests.
`pubspec.lock` is generated by Dart and should remain committed for this
application: its `https://pub.dev` entries, checksums, and transitive packages
make dependency resolution reproducible. They are not links to an old Node,
Tauri, or Rust build.

The two `path:` dependencies are also intentional. They select the narrowly
patched `media_kit` and `media_kit_video` copies under `third_party/`; removing
those paths would silently replace Zero's native playback and Linux teardown
fixes with the hosted packages. Packages such as `media_kit_libs_video` do not
appear in Dart imports because Flutter consumes them as native runtime plugins.

See [dependency maintenance](docs/dependencies.md) for the update cadence,
commands, native-package policy, and validation checklist.
...
for the full design, [local storage](docs/storage.md) for the platform data
layout, and [CI notes](docs/CI.md) for validation details.
